...
for the pilot; the Viber Web -> Mac -> Web open/edit/save/PNG workflow is verified locally.
...
Emoji fields, procedural sources, effect nodes, and repeat nodes use
`doc.global.seed + seedOffset`, so one node can be varied without changing the
whole document.
