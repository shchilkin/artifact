  ...
  if (layer.kind === 'primitive' || layer.kind === 'noise' || layer.kind === 'array') {
    return (
      <div className={sectionClassName}>
        ...
        <InspectorSlider label="X Position" value={Math.round(layer.x * 100)} min={-200} max={200} onChange={(value) => onChange({ x: value / 100 } as Partial<SourceLayer>)} />
        <InspectorSlider label="Y Position" value={Math.round(layer.y * 100)} min={-200} max={200} onChange={(value) => onChange({ y: value / 100 } as Partial<SourceLayer>)} />
        <ScaleLockRow
          scaleX={layer.scaleX}
          scaleY={layer.scaleY}
          locked={scaleLocked}
