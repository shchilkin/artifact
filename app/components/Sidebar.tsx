    ...
    <aside className="sidebar" ref={sidebarRef}>
      ...
      {selectedLayer && (
        <div className="sidebar-sections border-t border-border flex-shrink-0 max-h-[60%]">
            ...
            {selectedLayer.kind === 'effect' && (
              <>
                <Section title="LIGHT RAYS" hidden={!showEffectGroup(['rays', 'bloom', 'filmBurn', 'neonGlow', 'fog', 'speedLines'])} onRand={() => randomizeSelectedSection('RAYS')} onReset={() => resetSelectedSection('RAYS')}>
                  ...
                  {showEffectControl(['neonGlow']) && (
                    <>
                      <div className="flex justify-between items-center text-dim text-[10px]"><span>Neon Color</span><input type="color" value={selectedLayer.neonColor} onChange={(e) => applySelectedPatch<EffectLayer>({ neonColor: e.target.value })} className="w-9 h-7 border border-border rounded-sm p-0.5 bg-transparent cursor-pointer" /></div>
                      <Slider label="Neon Glow" value={selectedLayer.neonGlow} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ neonGlow: v })} effectKey="neonGlow" {...ip} />
                    </>
                  )}
                  {showEffectControl(['fog']) && (
                    <>
                      <div className="flex justify-between items-center text-dim text-[10px]"><span>Fog Color</span><input type="color" value={selectedLayer.fogColor} onChange={(e) => applySelectedPatch<EffectLayer>({ fogColor: e.target.value })} className="w-9 h-7 border border-border rounded-sm p-0.5 bg-transparent cursor-pointer" /></div>
                      <Slider label="Fog" value={selectedLayer.fog} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ fog: v })} effectKey="fog" {...ip} />
                    </>
                  )}
                  {showEffectControl(['speedLines']) && <Slider label="Speed Lines" value={selectedLayer.speedLines} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ speedLines: v })} effectKey="speedLines" {...ip} />}
                </Section>
                <Section title="GLITCH" hidden={!showEffectGroup(['glitch', 'rgbSplit', 'ca', 'interlace', 'dataMosh', 'vhsTracking'])} onRand={() => randomizeSelectedSection('GLITCH')} onReset={() => resetSelectedSection('GLITCH')}>
                  ...
                  {showEffectControl(['ca']) && <Slider label="Chrom. Ab." value={selectedLayer.ca} min={0} max={30} onChange={(v) => applySelectedPatch<EffectLayer>({ ca: v })} effectKey="ca" {...ip} />}
                  ...
                  {showEffectControl(['vhsTracking']) && <Slider label="VHS Tracking" value={selectedLayer.vhsTracking} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ vhsTracking: v })} effectKey="vhsTracking" {...ip} />}
                </Section>
                <Section title="TEXTURE" hidden={!showEffectGroup(['grain', 'scanlines', 'blur', 'matte', 'dither', 'emboss', 'linocut'])} onRand={() => randomizeSelectedSection('TEXTURE')} onReset={() => resetSelectedSection('TEXTURE')}>
                  ...
                  {showEffectControl(['matte']) && <Slider label="Matte" value={selectedLayer.matte} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ matte: v })} effectKey="matte" {...ip} />}
                  {showEffectControl(['dither']) && <Slider label="Dither" value={selectedLayer.dither} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ dither: v })} effectKey="dither" {...ip} />}
                  {showEffectControl(['emboss']) && <Slider label="Emboss" value={selectedLayer.emboss} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ emboss: v })} effectKey="emboss" {...ip} />}
                  {showEffectControl(['linocut']) && <Slider label="Linocut" value={selectedLayer.linocut} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ linocut: v })} effectKey="linocut" {...ip} />}
                </Section>
                ...
                <Section title="WARP" hidden={!showEffectGroup(['noiseWarp', 'morph', 'vortex', 'barrel', 'tear', 'mirror', 'wave', 'zoomBlur', 'warp', 'ripple', 'kaleidoscope', 'squeeze'])} onRand={() => randomizeSelectedSection('WARP')} onReset={() => resetSelectedSection('WARP')}>
                  ...
                  {showEffectControl(['wave']) && (
                    <>
                      <Slider label="Wave" value={selectedLayer.waveAmt} min={0} max={60} onChange={(v) => applySelectedPatch<EffectLayer>({ waveAmt: v })} effectKey="waveAmt" {...ip} />
                      <Slider label="Wave Freq" value={selectedLayer.waveFreq} min={1} max={12} onChange={(v) => applySelectedPatch<EffectLayer>({ waveFreq: v })} effectKey="waveFreq" {...ip} />
                    </>
                  )}
                  {showEffectControl(['zoomBlur']) && <Slider label="Zoom Blur" value={selectedLayer.zoomBlur} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ zoomBlur: v })} effectKey="zoomBlur" {...ip} />}
                  {showEffectControl(['ripple']) && (
                    <>
                      <Slider label="Ripple" value={selectedLayer.rippleAmt} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ rippleAmt: v })} effectKey="rippleAmt" {...ip} />
                      <Slider label="Ripple Freq" value={selectedLayer.rippleFreq} min={1} max={12} onChange={(v) => applySelectedPatch<EffectLayer>({ rippleFreq: v })} effectKey="rippleFreq" {...ip} />
                    </>
                  )}
                  {showEffectControl(['kaleidoscope']) && <Slider label="Kaleidoscope" value={selectedLayer.kaleidoscope} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ kaleidoscope: v })} effectKey="kaleidoscope" {...ip} />}
                  {showEffectControl(['squeeze']) && (
                    <>
                      <Slider label="Squeeze X" value={selectedLayer.squeezeX} min={-80} max={80} onChange={(v) => applySelectedPatch<EffectLayer>({ squeezeX: v })} effectKey="squeezeX" {...ip} />
                      <Slider label="Squeeze Y" value={selectedLayer.squeezeY} min={-80} max={80} onChange={(v) => applySelectedPatch<EffectLayer>({ squeezeY: v })} effectKey="squeezeY" {...ip} />
                    </>
                  )}
                </Section>
                <Section title="COLOR FX" hidden={!showEffectGroup(['hueShift', 'rgbSplit', 'vignette', 'pixelate', 'posterize', 'threshold', 'edgeDetect', 'gradientOverlay', 'sepia', 'infrared', 'color', 'solarize', 'bleachBypass', 'cyanotype', 'splitTone'])} onRand={() => randomizeSelectedSection('COLORFX')} onReset={() => resetSelectedSection('COLORFX')}>
                  ...
                  {showEffectControl(['sepia']) && <Slider label="Sepia" value={selectedLayer.sepia} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ sepia: v })} effectKey="sepia" {...ip} />}
                  {showEffectControl(['infrared']) && <Slider label="Infrared" value={selectedLayer.infrared} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ infrared: v })} effectKey="infrared" {...ip} />}
                  {showEffectControl(['solarize']) && <Slider label="Solarize" value={selectedLayer.solarize} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ solarize: v })} effectKey="solarize" {...ip} />}
                  {showEffectControl(['bleachBypass']) && <Slider label="Bleach Bypass" value={selectedLayer.bleachBypass} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ bleachBypass: v })} effectKey="bleachBypass" {...ip} />}
                  {showEffectControl(['cyanotype']) && <Slider label="Cyanotype" value={selectedLayer.cyanotype} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ cyanotype: v })} effectKey="cyanotype" {...ip} />}
                  {showEffectControl(['splitTone']) && (
                    <>
                      <Slider label="Split Tone" value={selectedLayer.splitToneAmt} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ splitToneAmt: v })} effectKey="splitToneAmt" {...ip} />
                      <div className="flex justify-between items-center text-dim text-[10px]"><span>Shadow Color</span><input type="color" value={selectedLayer.splitShadow} onChange={(e) => applySelectedPatch<EffectLayer>({ splitShadow: e.target.value })} className="w-9 h-7 border border-border rounded-sm p-0.5 bg-transparent cursor-pointer" /></div>
                      <div className="flex justify-between items-center text-dim text-[10px]"><span>Highlight Color</span><input type="color" value={selectedLayer.splitHighlight} onChange={(e) => applySelectedPatch<EffectLayer>({ splitHighlight: e.target.value })} className="w-9 h-7 border border-border rounded-sm p-0.5 bg-transparent cursor-pointer" /></div>
                    </>
                  )}
                ...
                <Section title="RISO" hidden={!showEffectGroup(['duotone', 'halftone', 'risoShift', 'overprint', 'riso'])} onRand={() => randomizeSelectedSection('RISO')} onReset={() => resetSelectedSection('RISO')}>
                  ...
                  {showEffectControl(['overprint']) && <Slider label="Overprint" value={selectedLayer.overprint} min={0} max={100} onChange={(v) => applySelectedPatch<EffectLayer>({ overprint: v })} effectKey="overprint" {...ip} />}
                </Section>
              </>
            )}
