...
export const EFFECT_META: Record<string, EffectMeta> = {
  ...
  dotGrain: {
    title: 'Dot Grain',
    description: 'Jittered round-dot grain that darkens tones into stippled texture.',
    valueLabel: 'dots 68%',
    goodFor: 'PS-style skull shading, stippled shadows, round retro texture',
    cfgOverride: { dotGrain: 68, dotGrainSize: 4, dotGrainDensity: 66, dotGrainJitter: 42 },
  },
  dotGrainSize: {
    title: 'Dot Size',
    description: 'Base diameter of each round grain dot in cover pixels.',
    valueLabel: '4px dots',
    cfgOverride: { dotGrain: 68, dotGrainSize: 4, dotGrainDensity: 66, dotGrainJitter: 42 },
  },
  dotGrainDensity: {
    title: 'Dot Density',
    description: 'How tightly the round grain dots are packed across the image.',
    valueLabel: 'density 66%',
    cfgOverride: { dotGrain: 68, dotGrainSize: 4, dotGrainDensity: 66, dotGrainJitter: 42 },
  },
  dotGrainJitter: {
    title: 'Dot Jitter',
    description: 'Randomness of the dot positions so the texture feels sampled instead of gridded.',
    valueLabel: 'jitter 42%',
    cfgOverride: { dotGrain: 68, dotGrainSize: 4, dotGrainDensity: 66, dotGrainJitter: 42 },
  },
  scanlines: {
    title: 'Scanlines',
  ...
  badStream: {
    title: 'Bad Stream',
    description: 'Low-bitrate stream failure with macroblocks, small compression blocks, smear, and dark dropouts.',
    valueLabel: 'compression 72%',
    goodFor: 'bad video calls, broken streams, night-vision compression, lo-fi screenshots',
    cfgOverride: {
      badStream: 72,
  ...
  macroblocks: {
    ...
    valueLabel: 'blocks 64px',
    goodFor: 'low-bitrate video, blocky screenshots, simple codec squares',
    cfgOverride: { badStream: 72, badStreamBlockSize: 64, badStreamDarkness: 18 },
  },
  detailBlocks: {
    title: 'Detail Blocks',
    description: 'Small codec fragments for crunchy low-bitrate texture inside the frame.',
    valueLabel: 'detail 86%',
    goodFor: 'subtle stream grit, small compression noise, crunchy details',
    cfgOverride: { badStream: 82, badStreamBlockSize: 54, badStreamDetail: 86, badStreamChroma: 28 },
  },
  blockSmear: {
    title: 'Block Smear',
    description: 'Copied neighboring blocks that smear the frame like a stalled stream.',
    valueLabel: 'smear 82%',
    goodFor: 'bad calls, frozen video, packet loss drag',
    cfgOverride: { badStream: 76, badStreamBlockSize: 52, badStreamSmear: 82 },
  },
  chromaBlocks: {
    title: 'Chroma Blocks',
    description: 'Blocky chroma compression and color drift, separate from RGB split.',
    valueLabel: 'chroma 86%',
    goodFor: 'color-compressed streams, digital poster noise, codec color breakup',
    cfgOverride: { badStream: 70, badStreamBlockSize: 46, badStreamDetail: 36, badStreamChroma: 86 },
  },
  blockDropout: {
    title: 'Block Dropout',
    description: 'Dark dropped blocks, like missing packets in a damaged stream.',
    valueLabel: 'dropout 86%',
    goodFor: 'packet loss, surveillance video, broken live streams',
    cfgOverride: { badStream: 78, badStreamBlockSize: 58, badStreamDarkness: 86 },
  },
  pixelStretch: {
    title: 'Pixel Stretch',
    description: 'Pulls pixels into directional streaks sampled from the source image.',
    valueLabel: 'stretch 70',
    goodFor: 'shader-style pixel pulls, scanline drags, stretched motion artifacts',
    cfgOverride: { pixelStretch: 70, pixelStretchLength: 34, pixelStretchAngle: 0 },
  },
  retroResolution: {
    title: 'Retro Resolution',
    description: 'Renders the upstream branch at a fixed low internal resolution, then nearest-upscales it.',
    valueLabel: '320px edge',
    goodFor: 'PS1-era frame buffers, old console covers, stable export pixel grids',
    cfgOverride: { retroResolution: 320 },
  },
  posterize: {
    title: 'Posterize',
  ...
  indexedPalette: {
    ...
    description: 'Maps pixels to the nearest swatch from a small editable palette while preserving transparency.',
    valueLabel: 'palette 100%',
    goodFor: 'console title screens, limited-color monsters, hot magenta/red/yellow scenes',
    cfgOverride: { indexedPalette: 100, indexedPaletteCount: 6 },
  },
  indexedPaletteCount: {
    title: 'Color Count',
    description: 'How many of the Indexed Palette swatches are active.',
    valueLabel: '6 colors',
    cfgOverride: { indexedPalette: 100, indexedPaletteCount: 6 },
  },
  gradientMap: {
    title: 'Gradient Map',
    description: 'Maps image luminance through a three-color ramp, then blends it with the original pixels.',
    valueLabel: 'map 80',
    goodFor: 'Figma-style shader color ramps, poster grading, synthetic tone remaps',
    cfgOverride: {
      gradientMap: 80,
  ...
  channelMixer: {
    title: 'Channel Mixer',
    description: 'Crossfeeds RGB channels into each other for matrix-like color separation.',
    valueLabel: 'mixer 70',
    goodFor: 'shader-style channel remaps, harsh color experiments, spectral photo shifts',
    cfgOverride: { channelMixer: 70, channelRedMix: 55, channelGreenMix: 35, channelBlueMix: 45 },
  },
  patternRefraction: {
    title: 'Pattern Refraction',
    description: 'Refracts the source through a repeating wave pattern.',
    valueLabel: 'refract 58',
    goodFor: 'glass texture bends, patterned displacement, Figma-style shader refraction',
    cfgOverride: { patternRefraction: 58, patternRefractionScale: 18, patternRefractionAngle: 35 },
  },
  sepia: {
    title: 'Sepia',
  ...
  bokehBlur: {
    title: 'Bokeh Blur',
    description: 'Softens the source with lens-like highlight lift around bright pixels.',
    valueLabel: 'bokeh 12px',
    goodFor: 'defocused photos, dreamy covers, soft shader-like light bloom',
    cfgOverride: { bokehBlur: 12, bokehThreshold: 68 },
  },
  hatching: {
    title: 'Hatching',
    description: 'Draws tone-aware line hatching over darker image regions.',
    valueLabel: 'hatch 65',
    goodFor: 'engraved linework, sketchy posters, shader-style illustration texture',
    cfgOverride: { hatching: 65, hatchScale: 9, hatchAngle: 35 },
  },
  gooeyMerge: {
    title: 'Gooey Merge',
    description: 'Blurs and thresholds alpha so nearby visible shapes merge into soft blobs.',
    valueLabel: 'merge 72',
    goodFor: 'liquid logo blobs, metaball-like masks, soft alpha merges',
    cfgOverride: { gooeyMerge: 72, gooeyRadius: 10, gooeyThreshold: 42 },
  },
  edgeDetect: {
    title: 'Edge Detect',
