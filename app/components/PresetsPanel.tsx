    ...
    <>
      ...
      <motion.div className="fixed top-0 right-0 bottom-0 w-[min(320px,100vw)] bg-sidebar border-l border-border flex flex-col z-300 overflow-hidden" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <div className="flex gap-2 px-4 py-2.5 border-b border-border shrink-0">
          <input type="text" placeholder="Preset name..." value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSave()} className="flex-1 bg-[oklch(14%_0.016_42)] border border-border text-text font-mono text-[11px] px-2 h-11 rounded-sm outline-none focus:border-accent placeholder:text-dim" />
        ...
        {presets.length === 0 ? (
          ...
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2 [scrollbar-width:thin] [scrollbar-color:var(--border)_transparent]">
            {presets.map((preset) => (
              <div key={preset.id} className="flex gap-2.5 p-2.5 border border-border rounded bg-[oklch(14%_0.016_42/0.5)] transition-colors hover:border-accent/30">
