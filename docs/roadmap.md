...
- **Shader Fills And Shader Effects** — keep shader work aligned with the
  ...
  | AI Custom Shaders | prompt-ready `AI Shader Effect` node requests OpenAI-backed validated `customSpec` JSON by default, offers deterministic local fallback only after explicit user choice, stores source provenance, processes a connected source through ordered procedural/source-aware operations, and uses durable idempotency, timeout handling, monthly quota accounting, provider request IDs, and token-usage logs; unconnected or empty AI effects render transparent | replace the constrained recipe with definition-backed generated shader code, then add generation history and monetary cost estimates/billing |
  ...
  | Shader Authoring Model | Shader Definition and Shader Instance are now distinct serialized concepts; AI, code, and presets are authoring methods while fill/effect are the only runtime roles | move AI generation from the constrained `customSpec` recipe to generated definition-backed shader code, then add reusable definition library/versioning |
