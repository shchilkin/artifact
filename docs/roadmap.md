...
- v0.47.0 was released on 2026-07-25 as the Artifact Canvas Chrome release.
  ...
  node-editor interaction performance gate stayed within budget. Tag `v0.47.0`
  resolves to production commit
  `840cd3bd740931be81c31f0d229cd415d003af8c`; exact-SHA staging, production
  API contract version 1, and production web verification passed before
  publication. All seven delivery issues and milestone #8 are closed. See
  [`version-plans/v0.47.md`](./version-plans/v0.47.md) and
...
- v0.45 was released on 2026-07-23 as the Artifact Editor Workflows release.
  ...
  style-guide specimens and focused desktop/mobile browser coverage. Tag
  `v0.45.0` resolves to production commit
  `09fa42c369361ee0ecb0aa0ba8f18058fa107678`; exact-SHA staging, production
...
- v0.42 was released on 2026-07-22 with the shared UI Foundation, distinct
  ...
  and production web verification passed before publication. v0.44 through
  v0.48 remain the active bounded migration program for Artifact product
  surfaces, editor workflows, inspectors, canvas
...
- v0.41.3 was released on 2026-07-21 as the isolated model-drop graph stability
  ...
  edges as a linear layer stack. The patch preserves the custom graph and
  inserts the model as a loose node at the drop point. Tag `v0.41.3` resolves
  to production commit `099cd87fac427cc37c101b9dfd20ce209d7def7c`; exact-SHA
  staging, production API, and production web verification passed. PR #199
  restored the product tree to the post-v0.41.2 boundary plus this fix; the
  experimental Artifact Runtime package and renderer work remain on their
  dedicated branch and are absent from the release. See
  [`releases/v0.41.3.md`](./releases/v0.41.3.md).
