...
independent runtime line `0.2.0-alpha.1`; the accepted portfolio experiment
...
- Grain, Scanlines, Glitch, and Chromatic Aberration effect layers;
...
environment, or shader nodes. It also rejects effect presets outside the four
...
The first slice can resolve its graph, images, basic layers, and four effects.
It must still reject the package for Noise Warp, Vortex, Tear, and the missing
font mapping. The current capability report contains seven layer-level issues:
three unsupported effects and four text layers sharing the unresolved font.
Those four blocker categories are the next bounded implementation target.
...
2. Add the exact Viber effect blockers through the shared renderer boundary,
   not a portfolio-only approximation.
3. Resolve the Viber font through a redistribution-safe checked-in font or an
   explicit host replacement and record that decision.
4. Render the retained Viber package in a clean browser with zero capability
   issues and compare it to the accepted static cover.
5. Measure first render, repeated render, and ten create/destroy cycles before
